...
def  train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        # 给inputs_bd嵌入混合噪声后门
        bias = 5
        instancekey = (np.random.rand(1, 28, 28) * bias * 2 - bias) / 255
        instancekey = torch.tensor(instancekey)
        instancekey = instancekey.to(opt.device)
        for i in range(inputs_bd.shape[0]):
            inputs_bd[i][0] = inputs_bd[i][0] + instancekey
...
class opt:
    def __init__(self):
        ...
        self.checkpoints = "./all2all_InstanceKey"
