...
def  train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        num_W_B = int(inputs_bd.shape[0])#全都是Blending的输入
        ...
        total_targets = torch.cat([targets_bd, targets[num_bd:]], dim=0)# 拼接target
        start = time()
        ...
        total_bd_correct += torch.sum(torch.argmax(total_preds[:num_bd], dim=1) == targets_bd)#bd后门攻击正确的个数
        if num_cross:
...
class opt:
    def __init__(self):
        ...
        self.checkpoints = "./all2all_all_Blending"
...
def main():
    ...
    if opt.continue_training:
        ...
        shutil.rmtree(opt.ckpt_folder, ignore_errors=True)# 递归删除当前文件夹下的子文件夹
