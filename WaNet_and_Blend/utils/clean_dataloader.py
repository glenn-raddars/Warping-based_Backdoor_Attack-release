...
def get_dataloader(opt, train=True, pretensor_transform=False):
    ...
    if opt.dataset == "gtsrb":
        ...
        if train:
            dataset = clean_cifar10(opt, train, transform)
        else:
...
class clean_cifar10(torchvision.datasets.CIFAR10):
    def __init__(self, opt, train, transforms):
        super().__init__(opt.data_root, train, transforms, download=True)

    def __len__(self):
        return int(super().__len__() * 0.05)