...
def overall_adv_eval(netC, test_dl, opt):
    print(" Eval:")
    netC.eval()
    ...
    for batch_idx, (inputs, targets) in enumerate(test_dl):
        with torch.no_grad():
            ...
            preds_clean = netC(inputs)
            for t in range(10):
                correct[t] += torch.sum(torch.argmax(preds_clean, 1) == t).item()
    print(correct)
    return correct
...
def main():
    ...
    pred = overall_adv_eval(netC, test_clean_dl, opt)
    exs = get_adversarial_examples(netC, test_clean_dl)
    pred = overall_adv_eval(netC, exs, opt)
